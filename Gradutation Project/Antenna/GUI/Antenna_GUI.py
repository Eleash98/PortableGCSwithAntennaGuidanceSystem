...
class Ui_Dialog(QtWidgets.QWidget):
    def setupUi(self, Dialog):
        ...
        Dialog.setStyleSheet("background-color: silver;")
        ...
        self.hightimer = QTimer(Dialog)
        self.hightimer.setInterval(100)
        self.hightimer.timeout.connect(self.setlow)
        self.lowtimer = QTimer(Dialog)
        self.lowtimer.setInterval(900)
        self.lowtimer.timeout.connect(self.sethigh)
        #################loading earth image######################
        global graphicsView
        global scene
        global pen
        global pen2
        global img 
        img = QtGui.QPixmap("earth_40.jpg")
        graphicsView = QtWidgets.QGraphicsView(Dialog)
        scene = QtWidgets.QGraphicsScene(Dialog)
        self.pixmap = QtWidgets.QGraphicsPixmapItem(img)
        scene.addItem(self.pixmap)
        pen = QtGui.QPen(QtCore.Qt.red)
        pen2 = QtGui.QPen(QtCore.Qt.yellow)
        pen.setWidth(7)
        pen2.setWidth(2)
        graphicsView.setGeometry(QtCore.QRect(600, 300, 773, 415))
        graphicsView.setScene(scene)
		##########################################################
        
        ###################connect pushbotton#####################
        self.Connect = QtWidgets.QPushButton(Dialog)
        self.Connect.setGeometry(QtCore.QRect(25, 180, 163, 23))
        self.Connect.setObjectName("Connect")
        self.Connect.clicked.connect(self.serialconnect)
        ##########################################################
        
        ####################Ping pushbotton#######################
        self.Ping = QtWidgets.QPushButton(Dialog)
        self.Ping.setGeometry(QtCore.QRect(25, 210, 163, 23))
        self.Ping.setObjectName("Ping")
        self.Ping.clicked.connect(self.pingtest)
        ##########################################################
        
        #################disconnect pushbotton####################
        self.Disconnect = QtWidgets.QPushButton(Dialog)
        self.Disconnect.setGeometry(QtCore.QRect(25, 240, 163, 23))
        ...
        self.Refresh.setGeometry(QtCore.QRect(25, 270, 163, 23))
        ...
        self.maximumattitude.setGeometry(QtCore.QRect(920, 240, 110, 25))
        ...
        self.label_30.setGeometry(QtCore.QRect(920, 215, 120, 20))
        ...
        self.longitude.setGeometry(QtCore.QRect(920, 180, 110, 25))
        ...
        self.label_31.setGeometry(QtCore.QRect(920, 155, 90, 20))
        ...
        self.latitude.setGeometry(QtCore.QRect(920, 120, 110, 25))
        ...
        self.label_32.setGeometry(QtCore.QRect(920, 95, 90, 20))
        self.label_32.setObjectName("label_32")
        self.label_32.setFont(QFont('Times',10,QFont.Bold))
        ##########################################################
        
        ###################satellite name field###################
        self.stname = QtWidgets.QTextBrowser(Dialog)
        self.stname.setGeometry(QtCore.QRect(920, 60, 110, 25))
        self.stname.setObjectName("stname")
        self.stname.setStyleSheet("background-color : white")
        self.label_33 = QtWidgets.QLabel(Dialog)
        self.label_33.setGeometry(QtCore.QRect(920, 35, 90, 20))
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_33.setText(_translate("Dialog", "Satellite Name"))
    ...
    def timeValues(self):
            ...
            if pingcount==5:
                    ...
                    if ser!=-1:
                            print ("found serial")
                            self.pingtest
            if sattrackflag==True:
                    if satplottrigger==True:
                            self.pixmap = QtWidgets.QGraphicsPixmapItem(img)
                            scene.addItem(self.pixmap)
                            graphicsView.setScene(scene)
                            self.maximumattitude.setText('%u'%(max(ATST)))
                            self.stname.setText('%s'%(str(satdf.loc[0,"STNAME"])))
                            for i in range (0,satlength-2):
                                    if xarr[i]<xarr[i+1]:
                                            graphicsView.scene().addLine(xarr[i], yarr[i],xarr[i+1], yarr[i+1],pen2)
                                    elif xarr[i]>xarr[i+1]:
                                            if xarr[i]>384:
                                                    graphicsView.scene().addLine(xarr[i], yarr[i],768, yarr[i+1],pen2)
                                            elif xarr[i]<384:
                                                    graphicsView.scene().addLine(0, yarr[i],xarr[i], yarr[i+1],pen2)
                    satpointcount+=1
                    graphicsView.setScene(scene)
                                            ...
                                            for i in range (0,satlength-2):
                                                    if xarr[i]<xarr[i+1]:
                                                            ...
                                                            if xarr[i]>384:
                                                                    ...
                                                                    graphicsView.scene().addLine(0, yarr[i],xarr[i], yarr[i+1],pen2)
                                            scene.addEllipse(xarr[j],yarr[j],7,7,pen)
                                            self.longitude.setText('%s'%(str(satdf.loc[j,"LONG"])))
                                            self.latitude.setText('%s'%(str(satdf.loc[j,"LAT"])))
                                            satplottrigger=False
                    if satpointcount==satlength-1:
                            satpointcount=0
                            sattrackflag=False
    def sndpressed(self):
                if ser!=-1:
...
class satpredictThread(QThread):
    def run(self):
            ...
            worksheet.write(0, 5,"STNAME" )
            worksheet.write(1, 5,satellitename )
            ROW=1
            while tstart<tend:
                    orb = Orbital(satellitename)
                    lonlataltarray=orb.get_lonlatalt(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(tstart)))
                    if lonlataltarray[0]<0:
                            lonvar=lonlataltarray[0]
                            lonvar*=-1
                            longstr=str(lonvar)
                            longstr+="W"
                    elif lonlataltarray[0]>0:
                    ...
                    if lonlataltarray[1]<0:
                            ...
                            latstr=str(latvar)
                            latstr+="S"
                    elif lonlataltarray[1]>0:
                            latstr=str(lonlataltarray[1])
                            latstr+='N'
