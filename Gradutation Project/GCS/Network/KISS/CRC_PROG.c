...
u16 CRC16_u16CCITTFalse(u8* Copy_u8PtrData, u16 Copy_u8DataLength)
{

	if(Copy_u8DataLength <= 0)
		return 0;
	...
	if(Copy_u8DataLength >= 1)
		B1 = Copy_u8PtrData[0];
	if(Copy_u8DataLength >= 2)
		B0 = Copy_u8PtrData[1];
	...
	for(Bindex=2;Bindex<Copy_u8DataLength+2;Bindex++)
	{
		/*Get the current byte of the data, if the data is finished, add two bytes of 0 */
		u8 CurrentByte = Bindex >= Copy_u8DataLength?	0:Copy_u8PtrData[Bindex];
		u8 i;
